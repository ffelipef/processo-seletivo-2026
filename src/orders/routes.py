...
# Importações de segurança centralizadas no security.py
from src.auth.utils import get_current_user, get_current_admin_user
from src.auth.models import User

from src.database import get_db
from src.orders import schemas
from src.orders.models import OrderStatus # 🚀 NOVO: Importação do Enum de status
...
# ==========================================
# ROTAS ESTÁTICAS (Devem vir antes do /{order_id})
# ==========================================
...
async def create_order_checkout(
    payload: schemas.CheckoutRequest = None,
    current_user: User = Depends(get_current_user), 
    db: AsyncSession = Depends(get_db),
    service: OrderService = Depends(get_order_service)
):
...
async def admin_list_all_orders(
    current_user: User = Depends(get_current_admin_user), # 🔒 Protegido: Apenas Admin!
    db: AsyncSession = Depends(get_db),
    service: OrderService = Depends(get_order_service)
):
...
async def simulate_payment_webhook(
    order_id: UUID,
    payment_in: schemas.PaymentSimulationRequest,
    current_user: User = Depends(get_current_user), 
    db: AsyncSession = Depends(get_db),
    service: OrderService = Depends(get_order_service)
):
    ...
    """
    updated_order = await service.handle_payment_status(order_id, payment_in.status, current_user.id, db)
...
async def update_order_status_admin(
    ...
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_admin_user), # 🔒 Protegido: Apenas Admin!
    service: OrderService = Depends(get_order_service)    # Injeção de dependência limpa
):
    """Atualiza o status de um pedido (ex: SHIPPED, DELIVERED). Rota restrita para Administradores."""
    updated_order = await service.update_status_by_admin(order_id, new_status, db)
