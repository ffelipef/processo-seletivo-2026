...
export const Route = createFileRoute("/checkout")({
  head: () => ({
    meta: [
      { title: "Caixa — NovaSphere" },
      { name: "description", content: "Finalize seu pedido na NovaSphere." },
    ],
  }),
...
function CheckoutPage() {
  ...
  async function onCheckout() {
    ...
    try {
      ...
      // 🚀 A SAÍDA DE MESTRE: Em vez de usar a navegação suave, 
      // forçamos o recarregamento da página para limpar o cache visual do React/Zustand
      window.location.href = `/success?orderId=${res.order_id}`;
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-12">
      <div className="flex items-center justify-between">
        <div>
          <DotBadge>Caixa / Terminal 01</DotBadge>
          <h1 className="font-display text-3xl font-semibold mt-1">Revisar e confirmar</h1>
        </div>
        <span className="font-dot text-xs text-muted-foreground">
          {auth.email ? `AUTH: ${auth.email}` : "SESSÃO ANÔNIMA"}
        </span>
      </div>
      ...
      <div className="mt-8 grid md:grid-cols-[1fr_360px] gap-6">
        <div className="grid-frame corner-ticks p-6">
          {cart.items.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="font-dot text-xl">NENHUM ITEM NA FILA</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left border-b border-border">
                  <th className="pb-2 font-dot text-[11px] tracking-widest text-muted-foreground uppercase">Unidade</th>
                  <th className="pb-2 font-dot text-[11px] tracking-widest text-muted-foreground uppercase text-right">Qtd</th>
                  <th className="pb-2 font-dot text-[11px] tracking-widest text-muted-foreground uppercase text-right">Subtotal</th>
                </tr>
              </thead>
              <tbody>
                {cart.items.map((it) => {
        ...
        <aside className="grid-frame corner-ticks p-6 h-fit sticky top-24">
          <DotBadge>Resumo</DotBadge>
          <div className="mt-4 space-y-2 text-sm">
            <Row label="Subtotal" value={`$${cart.total.toFixed(2)}`} />
            <Row label="Frete" value="Grátis" />
            {discountPreview !== null && (
              <Row label="Desconto" value={`-$${discountPreview.toFixed(2)}`} />
            )}
          </div>

          <div className="mt-6 border-t border-border pt-4">
            <label className="text-xs uppercase tracking-widest text-muted-foreground mb-2 block">Código de Desconto</label>
            ...
            {appliedCoupon && discountPreview !== null && (
              <p className="text-nova-red mt-2 text-[11px] uppercase tracking-wider">
                [{appliedCoupon}] -${discountPreview.toFixed(2)} aplicado
              </p>
            )}
          </div>

          <div className="border-t border-border mt-4 pt-4 flex items-baseline justify-between">
            <span className="font-dot text-xs uppercase tracking-widest text-muted-foreground">Total Est.</span>
